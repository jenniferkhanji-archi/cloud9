...
import { Utensils, Image, QrCode, Coffee, Sparkles } from "lucide-react";
...
export default async function AdminDashboardPage() {
  ...
  return (
    <div>
      ...
      {drinkName && (
        <Link href="/admin/drink-of-month" className="mt-6 block">
          <GlassCard className="flex items-start gap-3 p-4 transition-shadow hover:shadow-glass sm:p-5">
            <Coffee className="h-6 w-6 shrink-0 text-sky-blue" />
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-stone-600">This month&apos;s drink</p>
              <p className="mt-1 line-clamp-2 text-stone-800">{drinkName}</p>
            </div>
          </GlassCard>
        </Link>
      )}
